...
import { getGroupForScorer, updateScore } from '../services/supabaseMock';
import { Group, Course, GroupPlayer, Score } from '../types';
import { useRealtimeScorecard } from '../hooks/useRealtime';
...
const ScorecardScreen: React.FC<ScorecardScreenProps> = ({ accessCode, onBack }) => {
  const [initialData, setInitialData] = useState<{ group: Group, course: Course } | null>(null);
  ...
  const handleScoreChange = async (playerId: string, hole: number, change: number) => {
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-xl flex flex-col h-[calc(100vh-10rem)]">
        <div className="flex justify-between items-center pb-4 border-b border-gray-700">
            <Button onClick={onBack} variant="secondary" size="sm" className="flex items-center">
