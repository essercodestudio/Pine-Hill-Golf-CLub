...
import { AdminTournament, AdminPlayer, AdminGroup, AdminGroupPlayer, AdminHole, AdminTee } from '../../data/mockDatabase';
...
interface ManageGroupsProps {
  ...
  players: AdminPlayer[];
  groups: AdminGroup[];
  groupPlayers: AdminGroupPlayer[];
  holes: AdminHole[];
  tees: AdminTee[];
  onCreateGroup: (data: { tournamentId: string; startHole: number; playerIds: string[]; responsiblePlayerId: string; playerTeeSelections: Record<string, string>; }) => string;
}

const ManageGroups: React.FC<ManageGroupsProps> = ({ tournaments, players, groups, groupPlayers, holes, tees, onCreateGroup }) => {
  ...
  const [playerTeeColorSelections, setPlayerTeeColorSelections] = useState<Record<string, string>>({});
  ...
  const handleTeeColorSelectionChange = (playerId: string, color: string) => {
  ...
  const handleCreateGroup = (e: React.FormEvent) => {
    ...
    const playerTeeIdSelections: Record<string, string> = {};
  ...
  const getPlayerName = (playerId: string) => players.find(p => p.id === playerId)?.fullName || 'Desconhecido';
