...
import { AdminCourse, AdminHole, AdminTee } from '../../data/mockDatabase';
import Button from '../Button';

type TeeColor = 'Azul' | 'Branco' | 'Vermelho' | 'Dourado';
const teeColorOptions: TeeColor[] = ['Azul', 'Branco', 'Vermelho', 'Dourado'];
...
interface ManageCoursesProps {
  ...
  onCreateTeeForHole: (holeId: string, teeData: Omit<AdminTee, 'id' | 'holeId'>) => void;
}


const ManageCourses: React.FC<ManageCoursesProps> = ({ courses, holes, tees, onCreateCourse, onCreateTeeForHole }) => {
  ...
  const [newCoursePars, setNewCoursePars] = useState<number[]>(Array(18).fill(4));

  
  const [selectedCourseId, setSelectedCourseId] = useState<string | null>(null);
  const [newTeeForms, setNewTeeForms] = useState<Record<string, { color: TeeColor; yardage: string }>>({});
  ...
  const handleParChange = (index: number, value: string) => {
    ...
    newPars[index] = isNaN(par) ? 0 : par; // Store a number.
  ...
  const handleNewTeeFormChange = (holeId: string, field: 'color' | 'yardage', value: string) => {
    setNewTeeForms(prev => ({
      ...prev,
      [holeId]: {
  ...
  const handleAddTee = (holeId: string) => {
  ...
  const getTeesForHole = (holeId: string) => {
